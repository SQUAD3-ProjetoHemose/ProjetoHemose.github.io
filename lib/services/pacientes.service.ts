...
// IMPORTAR Paciente e DTOs de @/types
import { CreatePacienteDto, Paciente, UpdatePacienteDto } from '@/types';

export class PacientesService {
  // Buscar todos os pacientes
  static async getPacientes(): Promise<Paciente[]> {
    // Assumindo que pacientesAPI.getAll() retorna o tipo Paciente de @/types
    return pacientesAPI.getAll();
  }

  // Buscar paciente por ID
  static async getPacienteById(id: number): Promise<Paciente> {
    // Assumindo que pacientesAPI.getById() retorna o tipo Paciente de @/types
  ...
  static async getPacienteByCpf(cpf: string): Promise<Paciente> {
    ...
    // Assumindo que pacientesAPI.getByCpf() retorna o tipo Paciente de @/types
    return pacientesAPI.getByCpf(cpfLimpo) as Promise<Paciente>;
  }

  // Criar novo paciente
  static async criarPaciente(dados: CreatePacienteDto): Promise<Paciente> {
    // Usar CreatePacienteDto e assumir que pacientesAPI.create() espera esse tipo
    return pacientesAPI.create(dados);
  }

  // Atualizar paciente
  static async atualizarPaciente(id: number, dados: UpdatePacienteDto): Promise<Paciente> {
    // Usar UpdatePacienteDto e assumir que pacientesAPI.update() espera esse tipo
